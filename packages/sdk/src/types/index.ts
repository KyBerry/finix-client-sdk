...
type AlwaysRequiredFields<T extends FormType> = T extends "bank" ? "account_number" | "bank_code" | "account_type" : T extends "card" ? "number" | "expiration_date" | "security_code" : never;

// Fields that cannot be hidden
type NonHideableFields<T extends FormType> = T extends "bank" ? "account_number" | "bank_code" | "account_type" : T extends "card" ? "number" | "expiration_date" : never;

// Fields that don't support default values
type NonDefaultableFields<T extends FormType> = T extends "bank" ? "account_number" : T extends "card" ? "number" | "expiration_date" : T extends "token" ? "number" | "expiration_date" | "security_code" | "account_number" : never;

// Determine available field names based on form type and showAddress option
type AvailableFieldNames<T extends FormType, ShowAddress extends boolean = false> = BaseFieldMap[T] | (ShowAddress extends true ? AddressFieldName : never);

// Fields that can be hidden
type HideableField<T extends FormType, ShowAddress extends boolean = false> = Exclude<AvailableFieldNames<T, ShowAddress>, NonHideableFields<T>>;

// Fields that can be set as required (not always required)
type ConfigurableRequiredField<T extends FormType, ShowAddress extends boolean = false> = Exclude<AvailableFieldNames<T, ShowAddress>, AlwaysRequiredFields<T>>;

// Fields that can have default values
type DefaultableField<T extends FormType, ShowAddress extends boolean = false> = Exclude<AvailableFieldNames<T, ShowAddress>, NonDefaultableFields<T>>;
